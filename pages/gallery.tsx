		...
		<div id='GalleryDiv' className='container mx-auto px-6 py-16'>
			...
			<div className='fixed top-0 left-0 w-full z-50'>
				<div className='relative bg-gray-300 h-4'>
					...
					></div>
					<span className='absolute w-full text-center text-sm text-white'>{uploadProgress}%</span>
				</div>
			</div>
			<input id='file' type='file' name='file' accept='image/*' ref={inputRef} onChange={handleUploadOfImage} />

			{imageUpload && (
				<>
					<button className='m-4 w-fit h-fit rounded-sm bg-blue-500 text-white p-2' onClick={handleImageUpload}>
						Upload
					</button>
			...
			<Modal isOpen={!!selectedImage} onRequestClose={() => setSelectedImage(null)} contentLabel='Selected Image'>
				<div className='mt-12' style={{ position: 'relative', width: '90vw', height: '90vh' }}>
					{selectedImage && <OptimisedImage src={selectedImage} alt='Selected image' layout='contain' />}
				...
				{listFiles &&
					...
					listFiles.map((file: any, index: any) => (
						<Image
							...
							onClick={() =>
								handleThumbnailClick(`https://schipkeditbucket.s3.ap-southeast-2.amazonaws.com/${file.Key}`)
							}
						/>
					))}
			</div>
